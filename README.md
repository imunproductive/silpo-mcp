...
> Токен **і cookie** кабінету Сільпо теж потрапляють у `SILPO_MCP_SESSION_FILE`
> — тримайте файл приватним (cookie дозволяє оновлювати токен від імені акаунта).
...
- **Cookie (рекомендовано)** — `set_auth_cookie` зберігає cookie сесії
  `.AspNetCore.Identity.Application` (auth.silpo.ua), і сервер **сам оновлює**
  токен кабінету, коли той спливає (кожні ~24 год), без ручного втручання.
- **Токен** — `set_auth_token` приймає разовий Bearer-токен (живе ~24 год, далі
  треба вставити новий вручну).
...
**Де взяти cookie:** DevTools → Application → Cookies → `https://auth.silpo.ua`
